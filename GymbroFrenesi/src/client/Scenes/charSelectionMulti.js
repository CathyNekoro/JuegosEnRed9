...
export default class charSelectionMulti extends Phaser.Scene
{
    ...
    create()
    {
        this.add.rectangle(0, 0, this.cameras.main.width, this.cameras.main.width, 0x000000).setOrigin(0,0);
    ...
    onCharClicked(btn) {
        if (btn.locked) {console.log("[client] botón locked, return");return;}
        if (this.currentTurn !== this.yourId) { console.log("[client] no es tu turno, return");return; } // no es tu turno
    ...
    updateTurnUI() {
        ...
        console.log('[charSelectionMulti] this.players:', JSON.stringify(this.players));
    ...
    lockChosenButtons() {
        const chosenChars = [this.selected.player1, this.selected.player2].filter(c => c !== null);
        this.buttons.forEach(btn => {
    ...
    showOpponentLeftMessage() {
        ...
        this.add.rectangle(0, 0, this.cameras.main.width, this.cameras.main.height, 0x000000, 0.85)
            .setOrigin(0, 0);
        this.add.text(cx, cy - 50, "Tu rival ha abandonado la partida", {
            fontFamily: "Bubble",
            fontSize: "70px",
            color: "#ffffff"
        }).setOrigin(0.5);
