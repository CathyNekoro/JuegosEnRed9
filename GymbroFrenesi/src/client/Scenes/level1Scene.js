...
export default class level_1 extends Phaser.Scene {
  ...
  create() {
    ...
    this.inputMappings = InputConfig.map((config) => {
      return {
        ...
        quickAbilityKeyObj: this.input.keyboard.addKey(
          Phaser.Input.Keyboard.KeyCodes[config.quickAbilityKey],
        ),
        slowAbilityKeyObj: this.input.keyboard.addKey(
          Phaser.Input.Keyboard.KeyCodes[config.slowAbilityKey],
        ),
      };
    });
  ...
  getPlatformAt(gridX, gridY) {
    ...
    return (
      this.fallingPlatforms
        .getChildren()
        .find((p) => p.gridX === gridX && p.gridY === gridY) || null
    );
  }
  ...
  getRandomSafePlatform() {
    const candidates = this.fallingPlatforms
      .getChildren()
      .filter((p) => !p.fallen);
  ...
  update(time, delta) {
    ...
    this.inputMappings.forEach((mapping) => {
      ...
      const quickAbility = Phaser.Input.Keyboard.JustDown(
        mapping.quickAbilityKeyObj,
      );
      const slowAbility = Phaser.Input.Keyboard.JustDown(
        mapping.slowAbilityKeyObj,
      );
      ...
      const badTile = unavailableTiles.find(
        (tile) =>
