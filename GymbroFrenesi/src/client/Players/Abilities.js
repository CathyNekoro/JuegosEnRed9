...
export default class Abilities {
  ...
  useAbility() {
    ...
    if (this.scene.anims.exists(this.abilityKey) && !"coreDay_ability") {
      ...
      this.player.once("animationcomplete", () => {
        if (
          this.player.isAlive &&
          this.scene.anims.exists(this.player.idleingKey)
        ) {
  ...
  armQuickAbility() {
    ...
    this.scene.players.forEach((p) => {
      if (p.id !== this.player.id) {
        // comprobar que no es el mismo jugador
  ...
  coreQuickAbility() {
    ...
    while (!targetEnemy && this.player.isAlive) {
      ...
      if (badTile) {
        //si hay una casilla caida entre medias
    ...
    this.scene.players.forEach((enemy) => {
      if (enemy.id !== this.player.id) {
        ...
        adjacentTiles.forEach((tile) => {
          ...
          const targetTile = this.scene.map.getTileAt(
            tileGridX - 1,
            tileGridY - 1,
          );
          ...
          flashSprite.setScale(
            tileSize / flashSprite.width,
            tileSize / flashSprite.height,
          );
