export function createRoomService() {
  ...
  function createRoom(p1, p2) {
    ...
    console.log(
      `[Room] Creada sala ${roomId}: ${p1.nickName} vs ${p2.nickName}`,
    );
    return room;
  }
  ...
  const service = {
    joinQueue(socketId, nickName) {
      ...
      console.log(
        `[Room] ${nickName} entra en cola (cola actual: ${queue.length})`,
      );
    ...
    selectCharacter(socketId, character) {
      ...
      if (room.selectionState.phase !== "selecting")
        return { ok: false, error: "Selección ya cerrada" };

      const playerSlot =
        room.players[0].socketId === socketId ? "player1" : "player2";
      const otherSlot = playerSlot === "player1" ? "player2" : "player1";

      if (room.selectionState.turn !== playerSlot)
        return { ok: false, error: "No es tu turno" };
      if (room.selectionState.selections[playerSlot] !== null)
        return { ok: false, error: "Ya elegiste" };
      if (room.selectionState.selections[otherSlot] === character)
        return { ok: false, error: "Personaje ya elegido" };

      room.selectionState.selections[playerSlot] = character;
      console.log(`[Room] ${room.id}: ${playerSlot} eligió ${character}`);

      const bothPicked =
        room.selectionState.selections.player1 &&
        room.selectionState.selections.player2;
      if (bothPicked) {
